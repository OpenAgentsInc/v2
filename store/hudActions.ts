...
export function addPane(set: (fn: (state: PaneStore) => Partial<PaneStore>) => void, newPane: PaneInput, shouldTile = false) {
  return set((state) => {
    ...
    const newPaneWithPosition = createNewPaneWithPosition(newPane, newPane.id.toString(), adjustedPosition)
...
export function removePane(set: (fn: (state: PaneStore) => Partial<PaneStore>) => void, id: Pane['id']) {
  return set((state) => {
...
export function updatePanePosition(set: (fn: (state: PaneStore) => Partial<PaneStore>) => void, id: Pane['id'], x: number, y: number) {
  return set((state) => {
...
export function updatePaneSize(set: (fn: (state: PaneStore) => Partial<PaneStore>) => void, id: Pane['id'], width: number, height: number) {
  return set((state) => {
    const updatedPane = state.panes.find(pane => pane.id.toString() === id.toString())
    return {
      panes: state.panes.map(pane =>
...
export function bringPaneToFront(set: (fn: (state: PaneStore) => Partial<PaneStore>) => void, id: Pane['id']) {
  return set((state) => {
...
export function setActivePane(set: (fn: (state: PaneStore) => Partial<PaneStore>) => void, id: Pane['id']) {
  return set((state) => ({
    panes: state.panes.map(pane => ({
      ...pane,
